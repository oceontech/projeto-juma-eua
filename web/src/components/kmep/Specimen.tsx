...
/**
 * A cena de partículas do KMEP Ultra® — o mecanismo da cena do Aminosan
 * (`aminosan-b/Specimen.tsx`), com a matéria do KMEP.
 *
 * O hero fica como está. Quando o scroll começa, o texto dele sai, a foto da
 * bombona dá um zoom curto no rótulo e, ainda dentro do zoom, se fragmenta a
 * partir de si mesma. Dali a nuvem passa por quatro leituras — os desenhos de
 * `lib/scan/kmep.ts` —, cada uma com três chamadas penduradas no desenho, e
 * fecha num disco preto que a seção seguinte (`Blackout.tsx`) abre.
 *
...
/* Quantos pontos tem o pontilhado da foto — e, portanto, cada desenho: todo
   ponto da foto viaja e vira a forma, nada some no caminho. */
...
/* O rótulo da bombona, em fração da foto a partir do centro (y para cima).
   É para lá que o zoom anda. A foto do KMEP nasceu no mesmo enquadramento
   da do Aminosan: o rótulo cai no mesmo ponto. */
...
export function Specimen({
  ...
  const scene = variant === "b" ? content.sceneB : content.sceneA;
  const forms = KMEP_SCENES[variant];
      ...
      mm.add(
        ...
        (context) => {
            ...
            (async () => {
              ...
              try {
                /* A textura é o hero inteiro: o fundo com as folhas da frente
                   por cima (as mesmas do hero do Aminosan, como no Hero.tsx
                   daqui), compostos num canvas. Assim a foto que se fragmenta
                   é a mesma que o leitor estava olhando, com as duas camadas. */
                const suffix = narrow ? "-mobile" : "";
                const [bg, leaves] = await Promise.all([
                  loadImage(`/img/kmep/hero-bg${suffix}.webp`),
                ...
                if (!ctx) throw new Error("kmep scene: sem canvas 2D");
          ...
          if (process.env.NODE_ENV !== "production") {
            ...
            Object.assign(window, { kmepScene: u });
          }
          ...
          <div className="sp-slots grid">
            {stages.map((s, i) => (
              ...
              >
                ...
                {/* A legenda das cores do desenho: cobre é o que se perde,
                    lima é o que fica. A primeira leitura não tem cor. */}
                <ul className="sp-legend sp-fade mt-3 flex flex-wrap gap-x-5 gap-y-1 empty:hidden lg:mt-4">
                  {s.legend.map((l) => (
                    <li
                      key={l.label}
...
function loadImage(src: string) {
  return new Promise<HTMLImageElement>((resolve, reject) => {
    ...
    img.onerror = () =>
      reject(new Error(`kmep scene: falhou ao carregar ${src}`));
    img.src = src;
  });
}
