...
const NOZZLE_X = 150;
const FLIGHTS = [
  { to: 196, bend: 10, w: 3.2 },
  { to: 238, bend: 30, w: 2.6 },
  { to: 300, bend: 70, w: 2 },
  { to: 382, bend: 130, w: 1.5 },
  { to: 470, bend: 200, w: 1.1 },
  { to: 556, bend: 270, w: 0.8 },
];
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        gsap.fromTo(
          ...
          { strokeDashoffset: 1 },
          {
            ...
            stagger: 0.12,
            ease: "none",
            scrollTrigger: { trigger: ".cr-scene", start: "top 80%", end: "bottom 50%", scrub: 0.6 },
          },
        );
        gsap.to(".cr-air", { strokeDashoffset: -40, duration: 1.8, ease: "none", repeat: -1 });
      ...
      <div className="wrap grid gap-[clamp(40px,5vw,80px)] lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:items-center">
        ...
        <div className="cr-scene rounded-[clamp(14px,1.2vw,22px)] border border-offwhite/10 bg-linear-[122.93deg,var(--color-night-warm)_2.4%,var(--color-night-deep)_60.23%] p-[clamp(14px,2vw,28px)]">
          <svg viewBox="0 0 600 360" role="img" aria-label={credential.sceneAlt} className="w-full">
            ...
            {/* O ar. */}
            <g fill="none" stroke="#B9C2A0" strokeOpacity="0.45" strokeDasharray="10 14">
              {AIRFLOW.map((y) => (
                <path key={y} className="cr-air" d={`M${TUNNEL.x0 + 44} ${y} H${TUNNEL.x1 - 8}`} />
              ))}
            </g>
            ...
            <g fill="none" strokeLinecap="round">
              {FLIGHTS.map((f, i) => (
                <path
                  key={f.to}
                  className="cr-flight"
                  d={`M${NOZZLE_X} ${TUNNEL.top + 20} C${NOZZLE_X + f.bend * 0.15} ${TUNNEL.top + 120} ${f.to - f.bend * 0.25} ${TUNNEL.floor - 90} ${f.to} ${TUNNEL.floor - 14}`}
                  pathLength={1}
                  strokeDasharray="1 1"
            ...
            <g stroke="#EEEBE0" strokeOpacity="0.6" fill="none">
              {COLLECTORS.map((x) => (
                <path key={x} d={`M${x - 14} ${TUNNEL.floor - 14} V${TUNNEL.floor} H${x + 14} V${TUNNEL.floor - 14}`} />
              ))}
            </g>
      ...
      <div className="cr-context wrap mt-[clamp(72px,9vw,150px)] grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-center">
        ...
        <div className="relative grid grid-cols-3 gap-3 pt-6 lg:gap-4 lg:pt-0 lg:pb-[clamp(40px,5vw,80px)]">
          {context.stats.map((stat, i) => (
            <div
              key={stat.label}
              className="cr-card flex flex-col rounded-[clamp(12px,1.05vw,20px)] border border-offwhite/12 bg-offwhite/[0.05] p-[clamp(12px,1.6vw,24px)] backdrop-blur-md"
              style={{ marginTop: `calc(${i} * clamp(0px, 3vw, 48px))` }}
            >
              <span aria-hidden className="block h-[2px] w-8 rounded-full bg-offwhite/40" />
              ...
              <p className={`${microCaps} mt-2 text-[10px] text-offwhite/65`}>{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
