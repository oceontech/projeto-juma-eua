...
function keyframes(dir: 1 | -1, name: string, p: Required<CorridorPath>) {
  ...
  for (let s = 0; s <= p.stops; s++) {
    ...
    steps.push(
      // Each card carries its own perspective() around its centre, which sits
      // on the vanishing point, so no shared preserve-3d context is needed —
      // Chrome's depth sorting inside one clipped cards across the two rails.
      // Paint order follows depth through the animated z-index instead.
      `${(u * 100).toFixed(2)}%{z-index:${s};transform:perspective(${
        p.perspective
      }cqw) translate3d(${(dir * rail).toFixed(2)}cqw,0,${z.toFixed(
        2,
